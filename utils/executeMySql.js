...
const executeQuery = async (mysql) => {
    ...
    await result.then((value) => {
        ...
        writeTerminal({ message: err, nameFunction: "executeQuery", error: true });
    });
...
const connFunction = {
    ...
    */
    insert: async (tabella, rows) => {
        if (!tabella || !rows) {
            writeTerminal({ message: "Function Insert, dati non dichiarati in modo corretto", nameFunction: "insertQuery", error: true });
        }
        ...
        if (typeof rows === 'array') {
            ...
            writeTerminal({ message: "Invalid type of data: function Insert", nameFunction: "insertQuery", error: true });
        }
    ...
    */
    update: async (tabella, columns, whereCondition, arguments) => {
        if (!tabella || !columns || !whereCondition || !arguments) {
            writeTerminal({ message: "Function Update, dati non dichiarati in modo corretto", nameFunction: "updateQuery", error: true });
        }
        ...
        if (typeof columns === 'object') {
            ...
            writeTerminal({ message: "Invalid type of data: function Insert", nameFunction: "updateQuery", error: true });
        }
    ...
    */
    delete: async (tabella, whereCondition, arguments) => {
        if (!tabella || !whereCondition || !arguments) {
            writeTerminal({ message: "Function Update, dati non dichiarati in modo corretto", nameFunction: "deleteQuery", error: true });
        }
